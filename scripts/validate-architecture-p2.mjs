...
contains(mainIndex, 'aiRateLimiters', 'IPC context must receive rate limiters.')

const ipcHandlers = read('src/main/ipc/registerIpcHandlers.ts')
contains(ipcHandlers, 'aiRateLimiters: Partial<Record<ApiProvider, TokenBucketRateLimiter>>', 'IPC context must include rate limiter map.')
...
contains(ipcHandlers, 'limiter.acquire()', 'AI chat completion must acquire a rate limiter token.')
contains(ipcHandlers, 'settings.apiProvider === \'local\' ? null', 'Local provider must bypass remote rate limiting.')
contains(ipcHandlers, 'sanitizeAiErrorText(message, apiKey)', 'AI errors must be redacted after validation/network failures.')

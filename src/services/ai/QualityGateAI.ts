...
export class QualityGateAI {
  ...
  async generateQualityGateReport(
    ...
    const userPrompt = [
      ...
      'Each dimension must be 0-100. pass should be false if score < 80, if characterConsistency < 70, if characterStateConsistency < 70, if foreshadowingControl < 70, if chapterContinuity < 70, or if any high severity issue exists.',
